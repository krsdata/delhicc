<?php
/**
 * Register List Column Section.
 *
 * @package mag_news
 */

function mag_news_action_list_section() {

  register_widget( 'mag_news_list_section' );
  
}
add_action( 'widgets_init', 'mag_news_action_list_section' );

class mag_news_list_section extends WP_Widget{ 

	function __construct() {
		global $control_ops;
		$widget_ops = array(
		  'classname'   => 'business-section',
		  'description' => esc_html__( 'Add Widget to Display List Column Post.', 'mag-news' )
		);
		parent::__construct( 'mag_news_list_column_section',esc_html__( 'MN: List Column', 'mag-news' ), $widget_ops, $control_ops );
	}

	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, array( 
		  'title'				=> esc_html__( 'Business', 'mag-news' ),		
		  'category'         	=> '',
		  'number'           	=> 4, 
		  'show_post_meta'	 	=> true,
		  'show_category'	 	=> true,
    ...
    function widget( $args, $instance ) {
    	...
    	extract( $args );   
    	$title = ( ! empty( $instance['title'] ) ) ? esc_html($instance['title']) : esc_html__( 'Trending','mag-news' ); 	
	        ...
	        if ($the_query->have_posts()) : $count= 0; ?>
                ...
                <div class="post-layout-2">
                	<?php while ( $the_query->have_posts() ) : $the_query->the_post(); 
	                    ...
	                    <article class="featured-post post hentry <?php echo esc_attr( $imag_class );?>">
	                    	<?php if ( has_post_thumbnail() ): ?>
		                        <figure class="featured-post-image ">
		                            <?php the_post_thumbnail( 'mag-news-list-column' );?>
		                        </figure>
	                        <?php endif; ?>
