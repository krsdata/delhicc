 ...
 */
if( ! class_exists( 'Everest_News_lite_Fullwidth_Widget_Cat_Three' ) ) {
    
    class Everest_News_lite_Fullwidth_Widget_Cat_Three extends WP_Widget {
 
        function __construct() { 

            parent::__construct(
                'everest-news-lite-fullwidth-widget-cat-three',  // Base ID
                esc_html__( 'EN: Fullwidth Widget 3', 'everest-news-lite' ),   // Name
                array(
                    'description' => esc_html__( 'Displays posts. Use this widget at full-width top & bottom news area.', 'everest-news-lite' ), 
                )
            ); 
        }
        ...
        public function widget( $args, $instance ) {
            ...
            $post_cat_three = !empty( $instance[ 'post_cat_three' ] ) ? $instance[ 'post_cat_three' ] : 0;
            ...
            if( !empty( $post_cat_three ) ) {
                $post_args_three['cat'] = absint( $post_cat_three );
            }

            if( absint( $post_no ) > 0 ) {
                $post_args_one['posts_per_page'] = absint( $post_no );
                $post_args_two['posts_per_page'] = absint( $post_no );
                $post_args_three['posts_per_page'] = absint( $post_no );
            }
            ...
            $post_query_three = new WP_Query( $post_args_three );

            if( $post_query_one->have_posts() || $post_query_two->have_posts() || $post_query_three->have_posts() ) {
                ?>
                <section class="en-front-widget-cat-3 en-standard-section-spacing">
                    <div class="widgets-contents-entry">
                        <div class="row">
                            ...
                            if( $post_query_three->have_posts() ) {
                                ?>
                                <div class="en-col">
                                    <?php
                                    if( !empty( $title_three ) ) {
                                        ?>
                                        <div class="widget-title">
                                            <h3><?php echo esc_html( $title_three ); ?></h3>
                                        </div><!-- .widget-title -->
                                        <?php
                                    }
                                    ...
                                    while( $post_query_three->have_posts() ) {
                                        $post_query_three->the_post();
        ...
        public function form( $instance ) {
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_name('title_one') ); ?>">
                    <strong><?php esc_html_e('First Title', 'everest-news-lite'); ?></strong>
                </label>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'post_cat_one' ) )?>"><strong><?php echo esc_html__( 'Select First Category: ', 'everest-news-lite' ); ?></strong></label>
                <?php
                    $cat_args = array(
                        'orderby'   => 'name',
                        'hide_empty'    => 0,
                        ...
                        'show_option_all'   => esc_html__( 'Show Recent Posts', 'everest-news-lite' )
                    );
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_name('title_two') ); ?>">
                    <strong><?php esc_html_e('Second Title', 'everest-news-lite'); ?></strong>
                </label>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'post_cat_two' ) )?>"><strong><?php echo esc_html__( 'Select Second Category: ', 'everest-news-lite' ); ?></strong></label>
                <?php
                    $cat_args = array(
                        'orderby'   => 'name',
                        'hide_empty'    => 0,
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_name('title_three') ); ?>">
                    <strong><?php esc_html_e('Third Title', 'everest-news-lite'); ?></strong>
                </label>
                <input class="widefat" id="<?php echo esc_attr( $this->get_field_id('title_three') ); ?>" name="<?php echo esc_attr( $this->get_field_name('title_three') ); ?>" type="text" value="<?php echo esc_attr( $instance['title_three'] ); ?>" />   
            </p>

            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'post_cat_three' ) )?>"><strong><?php echo esc_html__( 'Select Third Category: ', 'everest-news-lite' ); ?></strong></label>
                <?php
                    $cat_args = array(
                        'orderby'   => 'name',
                        'hide_empty'    => 0,
                        'id'    => $this->get_field_id( 'post_cat_three' ),
                        'name'  => $this->get_field_name( 'post_cat_three' ),
                        'class' => 'widefat',
                        'taxonomy'  => 'category',
                        'selected'  => $instance['post_cat_three'],
                        'show_option_all'   => esc_html__( 'Show Recent Posts', 'everest-news-lite' )
                    );
                    wp_dropdown_categories( $cat_args );
                ?>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_name('post_no') ); ?>">
                    <strong><?php esc_html_e('No of Posts', 'everest-news-lite'); ?></strong>
                </label>
        ...
        public function update( $new_instance, $old_instance ) {
            ...
            $instance['post_cat_three']   = absint( $new_instance['post_cat_three'] );
    ...
    add_action( 'widgets_init' , function() {
        ...
        register_widget( 'Everest_News_Lite_Fullwidth_Widget_Cat_Three' );
