...
if ( ! function_exists( 'zigcy_lite_feat_prod_cat_setting' ) ) { 
    ...
    <div class="container">
        ...
        <div class="store-mart-lite-feat-prod-cat-wrap <?php if(!$ft_show_rating){echo esc_attr($class);} ?>">
            ...
            if(empty($zigcy_lite_feature_product_categories) || !in_array($zigcy_lite_feature_product_categories,$woo_cat) ){
               $category_p = max(array_keys($woo_cat)); //gets index which has maximum products
               $zigcy_lite_feature_product_categories = $woo_cat[$category_p];
